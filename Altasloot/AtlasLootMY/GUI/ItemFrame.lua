		...
		-- refresh title with diff and add pagenumber if there
			if #items and items[#items] and items[#items][1] > 100 then
				if not diffData.textIsHidden then
					...
					GUI.frame.contentFrame.title:SetText(format(PAGE_NAME_PAGE, GUI.frame.contentFrame.title.txt or "", AtlasLoot.db.GUI.selected[5]+1, floor(items[#items][1]/100)+1))
				end
			else
				if not diffData.textIsHidden then
