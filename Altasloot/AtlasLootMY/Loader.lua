...
local ATLASLOOT_MODULE_LIST = {
	{
		addonName = "AtlasLootMY_DungeonsAndRaids",
	...
	{
		addonName = "AtlasLootMY_Crafting",
		--icon = "Interface\\ICONS\\Inv_ChampionsOfAzeroth",
		name = AL["Crafting"],
		tt_title = nil,		-- ToolTip title
		tt_text = nil,		-- ToolTip text
	...
	{
		addonName = "AtlasLootMY_Factions",
		--icon = "Interface\\ICONS\\Inv_ChampionsOfAzeroth",
		name = AL["Factions"],
		tt_title = nil,		-- ToolTip title
		tt_text = nil,		-- ToolTip text
	...
	{
		addonName = "AtlasLootMY_Collections",
		--icon = "Interface\\ICONS\\Inv_ChampionsOfAzeroth",
		name = AL["Collections"],
		tt_title = nil,		-- ToolTip title
		tt_text = nil,		-- ToolTip text
...
LoaderFrame:SetScript("OnUpdate", function(self, elapsed)
    ...
    if AtlasLoot.db.standaloneMinimapIcon then
        if AtlasLoot:uncollectMMB() then
			 LoaderFrame:SetScript("OnUpdate", nil)
		end
    else
...
function AtlasLoot:uncollectMMB()
    if _G.U1CoreAPI and _G.U1CoreAPI.GetMMBFrame() then
        _G.U1CoreAPI.GetMMBFrame():__UnCollectButton(_G["LibDBIcon10_AtlasLootMY"])
        _G.U1CoreAPI.GetMMBFrame():__UpdateFunc();
		return true
    end
    local M = LibStub("AceAddon-3.0"):GetAddon("MBB", true);
    if M and type(M) == "table" then
        -- 删除OnUpdate事件处理器
        local iconName = "LibDBIcon10_AtlasLootMY"
        if _G[iconName] and _G[iconName].opoint then
            M:addignored("LibDBIcon10_AtlasLootMY")
        else
            M:addignored("_AtlasLootMY")
        end
        return true
...
function AtlasLoot:recollectMMB()
    if _G.U1CoreAPI and _G.U1CoreAPI.GetMMBFrame() then
		-- /dump _G["LibDBIcon10_AtlasLootMY"]:SetParent(_G.U1CoreAPI.GetMMBFrame()._Packer)
        _G.U1CoreAPI.GetMMBFrame():__CollectButton(_G["LibDBIcon10_AtlasLootMY"])
		_G.U1CoreAPI.GetMMBFrame():__UpdateFunc();
		return true
    end
    local M = LibStub("AceAddon-3.0"):GetAddon("MBB", true);
    if M and type(M) == "table" then
        ...
        local iconName = "LibDBIcon10_AtlasLootMY"
        if _G[iconName].opoint then
            M:removeignored("LibDBIcon10_AtlasLootMY")
        else
            M:removeignored("_AtlasLootMY")
        end
        return ture
...
function Loader.Init()
	...
	for i=1,GetNumAddOns() do
		...
		if tmp[1] and str_find(tmp[1], "AtlasLootMY_") then
			ModuleList[tmp[1]] = {
				index = i,
				...
				moduleName = GetAddOnMetadata(tmp[1], "X-AtlasLootMY-ModuleName") or tmp[1],
				lootModule = GetAddOnMetadata(tmp[1], "X-AtlasLootMY-LootModule"),
			}
		elseif tmp[1] and str_find(tmp[1], "AtlasMY_") then
	...
	if ModuleList["AtlasLootMY_Maps"] and ModuleList["AtlasLootMY_Maps"].enabled then
		Loader:LoadModule("AtlasLootMY_Maps", function() AtlasMapsModuleLoaded = true end)
	else
