...
local function ChatFilterFunc(_, event, msg, player, l, cs, t, flag, channelId, ...)
    ...
    local done;
    repeat
        local start, finish, characterName, displayName = remaining:find("%[AtlasLootMY: ([^%s]+) %- (.*)%]");
        if (characterName and displayName) then
            characterName = characterName:gsub("|c[Ff][Ff]......", ""):gsub("|r", "");
            displayName = displayName:gsub("|c[Ff][Ff]......", ""):gsub("|r", "");
            newMsg = newMsg .. remaining:sub(1, start - 1);
            newMsg = newMsg .. "|Hgarrmission:AtlasLootMY_fav|h|cFF8800FF[" .. characterName .. " |r|cFF8800FF- " .. displayName .. "]|h|r";
...
function Favourites.OnInitialize()
    ...
    hooksecurefunc("SetItemRef", function(link, text)
        if (link == "garrmission:AtlasLootMY_fav") then
            local _, _, characterName, displayName = text:find("|Hgarrmission:AtlasLootMY_fav|h|cFF8800FF%[([^%s]+) |r|cFF8800FF%- (.*)%]|h");
            if (characterName and displayName) then
                characterName = characterName:gsub("|c[Ff][Ff]......", ""):gsub("|r", "");
                displayName = displayName:gsub("|c[Ff][Ff]......", ""):gsub("|r", "");
                if (IsShiftKeyDown()) then
                    ...
                    if (editbox) then
                        editbox:Insert("[AtlasLootMY: " .. characterName .. " - " .. displayName .. "]");
                    end
                else
                ...
                ShowTooltip({
                    { 1, "AtlasLootMY", 0.5, 0, 1 },
                    { 1, AL["Malformed link"], 1, 0, 0 }
                });
            end
        end
    end)
    Comm:RegisterComm("AtlasLootMY", function(prefix, text, channel, sender)
        -- Comm received
        ...
        if (cmd == "sendList") and ChatLinkPending then
            ...
            if ChatLinkPending ~= listIdent then
                ...
                ShowTooltip({
                    { 2, "AtlasLootMY", listName, 0.5, 0, 1, 1, 1, 1 },
...
function Favourites:RequestList(characterName, listName)
    ...
    if (ChatLinkPending == listIdent) and ChatLinkData then
        ...
        ChatLinkData = false
        Comm:SendCommMessage("AtlasLootMY", "requestList:"..listName, "WHISPER", characterName)
        ShowTooltip({
            { 2, "AtlasLootMY", listName, 0.5, 0, 1, 1, 1, 1 },
...
function Favourites:SendListData(characterName, listName, listId, listIsGlobal)
    ...
    Comm:SendCommMessage("AtlasLootMY", "sendList:"..listName..":"..listContent, "WHISPER", characterName)
end
